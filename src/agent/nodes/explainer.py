    ...
    for org in orgs:
        ...
        if (is_advanced or is_ultra) and client_neo4j:
            try:
                ...
                LIMIT 5
                """
                records = await client_neo4j.execute_query(
                    project_query, {"org_name": org.canonical_name}
                )
                if records:
                    project_list_str = "\n".join(
                        [
                ...
                print(
                    f"   [Advanced/Ultra Mode] Warning: Failed to query projects for {org.canonical_name}: {e}"
                )

        if is_ultra and project_list_str:
            print(
                f"   [Ultra Mode] Running multi-loop deep reasoning, critique, and proposal strategy for {org.canonical_name}..."
            )
...
"""
            try:
                resp_suitability = await client.chat_model.ainvoke(
                    [HumanMessage(content=prompt_suitability)]
                )
            ...
            if callback:
                await callback(
                    "info", f"[{org.canonical_name}] Loop 2: Reviewer risk & gap analysis..."
                )
            prompt_critique = f"""
...
"""
            try:
                resp_critique = await client.chat_model.ainvoke(
                    [HumanMessage(content=prompt_critique)]
                )
            ...
            if callback:
                await callback(
                    "info", f"[{org.canonical_name}] Loop 3: Formulating proposal strategy..."
                )
            prompt_strategy = f"""
...
"""
            try:
                resp_strategy = await client.chat_model.ainvoke(
                    [HumanMessage(content=prompt_strategy)]
                )
            ...
            if callback:
                await callback(
                    "info", f"[{org.canonical_name}] Loop 4: Synthesizing score and report..."
                )
            prompt_synthesis = f"""
...
"""
            try:
                resp_synthesis = await client.chat_model.ainvoke(
                    [HumanMessage(content=prompt_synthesis)]
                )
            ...
            print(
                f"   [Advanced Mode] Running deep reasoning matching and project alignment for {org.canonical_name}..."
            )
            if callback:
                await callback(
                    "info",
                    f"[{org.canonical_name}] Running deep project matching & gap analysis...",
                )
            prompt = f"""
You are an expert GSoC mentor for the organization '{org.canonical_name}'.
You are evaluating a student with the following profile:
...
"""
        ...
        try:
            ...
            if (is_advanced or is_ultra) and "### RECOMMENDATION SCORE" in explanation:
                ...
                if match:
                    ...
                    print(
                        f"   [{'Ultra' if is_ultra else 'Advanced'} Mode] Parsed LLM recommendation score: {val}/100 ({llm_score_raw:.2f}/10)"
                    )
            ...
            print(
                f"   Warning: Failed to generate explanation for {org.canonical_name}. Error: {e}"
            )
    ...
    return {"ranked_organizations": orgs, "match_result": result.model_dump()}
