...
class VectorSearcher:
    ...
    async def search_organizations(self, query: str, limit: int = 10, required_years: list[int] | None = None) -> list[dict]:
            ...
            query_filter = models.Filter(
                must=[
                    models.FieldCondition(
                        key="years_active",
                        match=models.MatchAny(any=required_years)
                    )
                ]
            )
    ...
    async def search_projects(self, query: str, limit: int = 10, org_name: str | None = None) -> list[dict]:
